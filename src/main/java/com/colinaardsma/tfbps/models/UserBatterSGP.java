...
public class UserBatterSGP extends AbstractEntity {
	...
	public UserBatterSGP(List<FPProjBatter> batterList, FPProjBatter batter, YahooRotoLeague yahooRotoLeague, User user) {
		this.batter = batter;
...
//    	BigDecimal dollarBatterSGP = new BigDecimal(batterList.get(bOverOneDollar.intValue()).getOpsTotalSGP());
    	...
    	BigDecimal dollarPerSGP = leagueBudgetB.divide(totalSGPb);
    	
    	double aav = new BigDecimal(batter.getOpsTotalSGP()).multiply(dollarPerSGP).doubleValue();
    	return aav;
